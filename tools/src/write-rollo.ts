...
function parseRequest(value: unknown): WriteRolloRequest {
...
function contentRow(paragraph: string): string {
  const image = /^!\[(.*?)]\((.*?)\)$/.exec(paragraph);
  if (image?.[2]?.trim()) {
    const alt = image[1]?.trim() ?? "";
    const src = image[2].trim();
    return [
      "      <tr>",
      `        <td><img src="${escapeHtml(src)}" alt="${escapeHtml(alt)}"></td>`,
      "      </tr>",
    ].join("\n");
  }
  ...
  return [
    "      <tr>",
    `        <td class="textorollo">${renderText(paragraph)}</td>`,
    "      </tr>",
  ].join("\n");
}

export function renderRolloHtml(request: WriteRolloRequest): string {
  ...
  return [
    ...
    "    <table>",
    "      <tr>",
    `        <td class="titulorollo">${renderText(request.titulo)}</td>`,
    "      </tr>",
    contentRows,
    authorRow,
    "    </table>",
...
function existingAuthor(html: string): string | null {
  ...
  while ((match = cells.exec(html)) !== null) {
    ...
    if (!classNames.split(/\s+/).includes("textoderecha")) continue;
...
export async function writeRolloHtml(
  rawRequest: unknown,
  docsDir: string,
): Promise<string> {
  ...
  const target = path.resolve(
    docsDir,
    `detallerollo.php-id=${request.id}&pagina=${request.pagina}.htm`,
  );

  await withFileLock(target, async () => {
    const targetExists = await exists(target);
    if (request.operation === "create" && targetExists) {
      throw new Error(`El divino rollo ya existe: ${target}`);
    }
    if (request.operation === "replace" && !targetExists) {
      throw new Error(`No existe el divino rollo que se quiere reemplazar: ${target}`);
    }
    ...
    await writeAtomic(target, renderRolloHtml(requestToRender));
  });
...
async function main(): Promise<void> {
  ...
  const target = await writeRolloHtml(JSON.parse(raw) as unknown, docsDir);
  console.log(`Divino rollo guardado: ${target}`);
}
